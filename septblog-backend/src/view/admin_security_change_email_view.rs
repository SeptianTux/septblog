...
fn validation(data: &FormData) -> Result<bool, crate::error::Error> {
    let mut error = false;
    let mut error_code = 0;
    let mut error_message = String::new();

    if data.new_email_address.is_empty() {
        error = true;
        error_code = 45;
        error_message.push_str("Empty email address.");
    }
...
pub fn put(
    ...
    let res = match crate::viewmodel::admin_security_change_email_view_model::put(&pool, &req, &config, &data) {
        Ok(val) => val,
        Err(err) => {
            ret["response"]         = false.into();
            ret["error"]            = true.into();
    ...
    if res.is_some() {
        ...
        ret["access_token"] = res.unwrap().into();
    } else {
        ret["response"]     = false.into();
    }
