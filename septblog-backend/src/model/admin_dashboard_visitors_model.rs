...
pub fn get_visitors_from_database(
    ...
    article_author: &u64
) -> Result<Vec<crate::viewmodel::admin_dashboard_visitors_view_model::Visitor>, crate::error::Error> {
    ...
    let visitors = conn.exec_map(
        ...
        |(article_title, article_id, article_author, ip_address, user_agent, referer, visited_at)| crate::viewmodel::admin_dashboard_visitors_view_model::Visitor {
            article_title,
            article_id,
            article_author,
            ip_address,
...
mod tests {
    ...
    fn get_visitors_from_database() {
        ...
        let result = match super::get_visitors_from_database(&db_pool, &article_author) {
            Ok(val) => val,
            Err(err) => {
                eprintln!("{:?}", err);
                return;
