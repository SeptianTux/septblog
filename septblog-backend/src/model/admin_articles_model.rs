...
pub fn get_article_data(
    pool: &std::sync::Arc<mysql::Pool>,
    author: &u64,
    page: &u32
) -> Result<Vec<crate::viewmodel::admin_articles_view_model::Article>, crate::error::Error> {
    ...
    let articles = conn.exec_map(
        "SELECT id, title, author, status, created FROM articles WHERE author = :article_author and status=1 OR status=2 ORDER BY counter DESC LIMIT :limit OFFSET :offset",
        params! {
            "article_author" => author,
            "limit" => per_page,
            "offset" => offset
        },
        |(id, title, author, status, created)| ArticleData {
            id,
            title,
            author,
            status,
    ...
    let mut ret: Vec<crate::viewmodel::admin_articles_view_model::Article> = Vec::new();
    ...
    for i in articless {
        ...
        let article = crate::viewmodel::admin_articles_view_model::Article {
            id: i.id,
            title: i.title,
...
mod tests {
    ...
    fn get_article_data() {
        ...
        let res = super::get_article_data(&db_pool, &author, &page);
