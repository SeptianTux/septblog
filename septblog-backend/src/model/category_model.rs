...
pub fn get_articles_with_this_category(
    ...
    let category_id_result = conn.exec_first(
    ...
    let category_id: String = match category_id_result {
        Ok(val) => {
            match val {
                Some(v) => v,
                None => {
                    log::error!("Data not found.");
                    log::debug!("Get category id return None value.");
        ...
        Err(err) => {
            log::error!("Failed to get category id from database.");
            log::debug!("{:?}", err);

            return Err(
                crate::error::Error {
                    code: 392,
                    message: "Failed to get category id from database.".to_string()
                }
            )
        }
    };

    let query = "SELECT article_id FROM article_categories WHERE category_id = :category_id";
    let params = params! {
        "category_id" => category_id
    };
    let article_ids: Result<Vec<String>, mysql::Error> = conn.exec(query, params);
    let result = match article_ids {
        Ok(val) => val,
        Err(err) => {
            log::error!("Failed to get category ids from database.");
            log::debug!("{:?}", err);

            return Err(
                crate::error::Error {
                    code: 389,
                    message: String::from("Failed to get category ids from database.")
                }
            );
        }
    };
...
pub fn get_articles(
    ...
    page: &u64
) -> Result<Vec<crate::viewmodel::category_view_model::ArticleWithNoAuthorInfo>, crate::error::Error> {
    let mut conn = match pool.get_conn() {
        Ok(con) => con,
        Err(e) => {
            let err: crate::error::Error = crate::error::Error {
    ...
    for i in 0..article_ids.len() {
        placeholders.push('?');
        if i < article_ids.len() - 1 {
            placeholders.push_str(", ");
        }
    }

    let per_page: u64 = 20;
    let offset: u64 = (page - 1) * per_page;

    let mut params: Vec<mysql::Value> = Vec::new();

    for id in article_ids {
        params.push(mysql::Value::from(id.clone()));
    }

    params.push(mysql::Value::from(per_page));
    params.push(mysql::Value::from(offset));

    let query = format!(
        "SELECT id, title, author, content, created FROM articles WHERE id IN ({}) AND status=2 ORDER BY counter DESC LIMIT ? OFFSET ?",
        placeholders
    );
    let result = conn.exec_map(
        query,
        mysql::Params::Positional(params),
        |(id, title, author, content, created)| crate::viewmodel::category_view_model::ArticleWithNoAuthorInfo { id, title, author, content, created },
    );
    ...
    for i in ret.iter_mut() {
        let cuted_article = article_cutter(i.content.clone());

        i.content = cuted_article;
    }
...
pub fn get_author_info_from_database(
    pool: &std::sync::Arc<mysql::Pool>,
    author_id: &u64
) -> Result<Option<crate::viewmodel::category_view_model::AuthorInfo>, crate::error::Error> {
    let mut conn = match pool.get_conn() {
        Ok(con) => con,
        Err(e) => {
            let err: crate::error::Error = crate::error::Error {
    ...
    let val = match result {
        Ok(val) => {
            match val {
                Some(v) => {
                    Some(
                        crate::viewmodel::category_view_model::AuthorInfo {
                            id: v.0,
                            first_name: v.1,
                            last_name: v.2,
                            username: v.3
