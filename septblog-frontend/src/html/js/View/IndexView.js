...
export class IndexView {
    ...
    async getArticlesFromBackend(page) {
        ...
        try {
            res = await this.viewModel.getArticlesFromBackend(page);
        } catch(error) {
    ...
    async #infiniteScrollListener() {
        window.addEventListener("scroll", () => {
            if (window.innerHeight + window.scrollY >= document.getElementById("homeArticles").offsetHeight + 80) {
    ...
    async #articles() {
        let articlesFromBackend = await this.getArticlesFromBackend(this.#homePageCounter);
        let articles = null;

        this.#increaseHomePageCounter();

        if (articlesFromBackend.data !== null) {
            articles = '';
            ...
            for (let i = 0; i < articlesFromBackend.data.length; i++) {
                ...
                articles = articles + '<div class="home-article border-bottom padding-12px">' + 
                                            '<div>' + 
                                                '<h2>' +
    ...
    async loadArticles() {
        ...
        if (articles === null) {
            if (this.#homePageCounter === 2) {
                articles = '<div class="home-articles-no-articles">There are no articles.</div>';
                document.getElementById("homeArticles").insertAdjacentHTML("beforeend", articles);
            }
        } else {
            document.getElementById("homeArticles").insertAdjacentHTML("beforeend", articles);
        }
    ...
    addInfiniteScrolSpinner() {
        const spinner = '<div id="spinnerHomeArticlesInfiniteScroll" class="spinner-home-articles-infinite-scroll spinner-border mx-auto d-block " role="status">' +
                            '<span class="visually-hidden">Loading...</span>' +
                        '</div>';

        document.getElementById("homeArticles").insertAdjacentHTML("beforeend", spinner);
    }

    removeInfiniteScrollSpinner() {
        if (this.isInfiniteScrollSpinnerExist()) {
            document.getElementById("spinnerHomeArticlesInfiniteScroll").remove();
        }
    }

    isInfiniteScrollSpinnerExist() {
        const spinner = document.getElementById("spinnerHomeArticlesInfiniteScroll");
